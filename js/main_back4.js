...
function step(){
	...
	for(var rIndex = 0; rIndex < rows; rIndex++){
		for(var cIndex = 0; cIndex < cols; cIndex++){
			if(grid[rIndex][cIndex].type == 0){
				if(startrow == -1 || startcol == -1){
					startrow = rIndex;
					startcol = cIndex;
				}
			}
			if(grid[rIndex][cIndex].type == 1){
	...
	for (var f = 0; f < rows; f++){
		for (var g = 0; g < cols; g++){
			...
			for (var s = 0; s < n.length; s++) {
				...
				if((openList.includes(grid[coord.y][coord.x])) == false && !(closedList.includes(grid[coord.y][coord.x]))){
					...
					//console.log("grid[" + coord.y + "][" + coord.x + "].f: " + grid[coord.y][coord.x].f);
					
					min = grid[coord.y][coord.x].f;
				}
			}
		}
		...
		for(var l = 0; l < rows; l++){
			for(var m = 0; m < cols; m++){
				if(grid[l][m].f <= min && openList.includes(grid[l][m]) && !(closedList.includes(grid[l][m]))){
					...
					var pop = openList.shift(grid[coord.y][coord.x]);
...
function draw(){
	for(var j in grid){
		for(var k in grid[j]){
			...
			context.fillText(grid[j][k].f,(k * (nDim + nBrd)) + 2, (j * (nDim + nBrd)) + 10);
		}
	}
}
